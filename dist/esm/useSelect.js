...
function multiInput(multiple, input, options, originOptions, onSelect) {
  if (multiple) {
    const multiValue = Array.isArray(input) ? input : input.replace(/\，/g, ",").split(","); // console.log("inside multi input and to call onselect");
...
export default function useSelectSearch({
  ...
  const ref = useRef(null); // console.log(arrayOptions);
  ...
  const {
    ...
    e,
    flat
  } = state; // console.log(`newoptions is ${JSON.stringify(newOptions)}`);

  const flatDefaultOptions = useMemo(() => flattenOptions(newOptions), [focus, newOptions, addedOptions, arrayOptions]); // console.log(`flat default options ${JSON.stringify(flatDefaultOptions)}`);
  ...
  useEffect(() => {
    ...
    var updatedOptions = multiInput(multiple, search, Options, [...newOptions], onSelect); // console.log("updated options is " + JSON.stringify(updatedOptions));
  ...
  const onSelect = useCallback((val, e) => {
    // console.log(state);
    // console.log(val);
    // console.log(e);
    setState(oldState => {
      ...
      const values = getNewValue(item, oldState.value, multiple); // console.log(values);
      // console.log(oldState);

      const newOptions = getOption(values, state.newOptions); // console.log(newOptions);
